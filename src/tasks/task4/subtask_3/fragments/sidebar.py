...
def show_sidebar():
    with st.sidebar:
        ...
        function = st.text_input('f(x):', value='x^4 + x^2 + 1')
        ...
        n = int(st.number_input('Количество частей:', value=40, min_value=1))
        StateVar.NUMBER_OF_SEGMENTS.set(n)

        l = int(st.number_input('Множитель l:', value=3, min_value=1))  # noqa: E741
        StateVar.MULTIPLIER.set(l)
